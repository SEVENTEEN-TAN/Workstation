...
import { editableTextProps, safeEditorLinkTarget, selectableFieldProps } from "./visual-editing";
...
export function Footer({ sectionNumber }: { sectionNumber?: string } = {}) {
  ...
  return (
    <footer id="contact" className="relative scroll-mt-20 overflow-hidden border-t border-white/10 bg-ink pb-10 pt-24 sm:pt-32">
      <div className="pointer-events-none absolute inset-x-0 bottom-[-3vw] overflow-hidden text-center" aria-hidden="true"><span {...editableTextProps(editor, `${locale}.footer.backdrop`)} className="pointer-events-auto font-display text-[25vw] font-black leading-none tracking-[-0.08em] text-white opacity-[0.035]">{copy.footer.backdrop}</span></div>
      <div className="page-shell relative z-10">
        <motion.div {...reveal} transition={{ duration: 0.7 }} className="grid gap-16 border-b border-white/10 pb-20 lg:grid-cols-2 lg:gap-24">
          ...
          <div className="grid grid-cols-2 gap-10 self-end sm:gap-20 lg:justify-self-end">
            ...
            <div><p {...editableTextProps(editor, `${locale}.footer.socials`)} className="mb-6 text-xs font-bold uppercase tracking-[0.22em] text-gray-600">{copy.footer.socials}</p><ul className="space-y-4 text-base text-gray-300"><li><a {...selectableFieldProps(editor, "settings.githubUrl")} href={editor ? safeEditorLinkTarget("github", settings.githubUrl) : settings.githubUrl} target="_blank" rel="noreferrer" className="focus-ring inline-flex items-center gap-2 transition-colors hover:text-accent"><Code2 className="size-4" /><span {...editableTextProps(editor, `${locale}.footer.github`)}>{copy.footer.github}</span></a></li><li className="group/wechat relative"><button type="button" aria-label={copy.footer.wechatHint} aria-describedby="wechat-qr" className="focus-ring inline-flex items-center gap-2 transition-colors hover:text-accent"><MessageCircle className="size-4" /><span {...editableTextProps(editor, `${locale}.footer.wechat`)}>{copy.footer.wechat}</span></button><div id="wechat-qr" className="invisible absolute bottom-full right-0 z-20 mb-4 w-44 translate-y-2 rounded-2xl border border-white/10 bg-white p-2 opacity-0 shadow-2xl shadow-black/60 transition-all duration-300 group-hover/wechat:visible group-hover/wechat:translate-y-0 group-hover/wechat:opacity-100 group-focus-within/wechat:visible group-focus-within/wechat:translate-y-0 group-focus-within/wechat:opacity-100"><img {...selectableFieldProps(editor, "settings.wechatQrImage")} src={settings.wechatQrImage} alt={copy.footer.wechatAlt} width="648" height="633" loading="lazy" className="h-auto w-full rounded-xl" /></div></li></ul></div>
          </div>
        </motion.div>
        <div className="relative z-10 flex flex-col gap-4 pt-8 text-xs text-gray-600 sm:flex-row sm:items-center sm:justify-between"><p {...editableTextProps(editor, `${locale}.footer.copyright`)}>{copy.footer.copyright}</p><div className="flex gap-6"><span {...editableTextProps(editor, `${locale}.footer.privacy`)}>{copy.footer.privacy}</span><span {...editableTextProps(editor, `${locale}.footer.terms`)}>{copy.footer.terms}</span></div></div>
      </div>
    </footer>
  );
}
