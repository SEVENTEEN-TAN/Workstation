    ...
    <footer id="contact" className="relative scroll-mt-20 overflow-hidden border-t border-white/10 bg-ink pb-10 pt-24 sm:pt-32">
      ...
      <div className="page-shell relative z-10">
        <motion.div {...reveal} transition={{ duration: 0.7 }} className="grid gap-16 border-b border-white/10 pb-20 lg:grid-cols-2 lg:gap-24">
          <div><p className="eyebrow">{sectionNumber ? copy.footer.eyebrow.replace(/^\d{2}/, sectionNumber) : copy.footer.eyebrow}</p><h2 aria-label={copy.footer.headingLabel} className="section-heading mt-7">{copy.footer.heading[0]}<br /><span className="text-accent">{copy.footer.heading[1]}</span></h2><p className="mt-7 max-w-lg text-base leading-relaxed text-gray-400 sm:text-lg">{copy.footer.intro}</p><a href={`mailto:${settings.email}`} className="focus-ring mt-9 inline-flex items-center gap-3 rounded-full bg-white px-6 py-3.5 text-sm font-bold text-black transition-transform duration-300 hover:scale-105 sm:px-7">{settings.email}<ArrowUpRight className="size-4" /></a></div>
          <div className="grid grid-cols-2 gap-10 self-end sm:gap-20 lg:justify-self-end">
            ...
            <div><p className="mb-6 text-xs font-bold uppercase tracking-[0.22em] text-gray-600">{copy.footer.socials}</p><ul className="space-y-4 text-base text-gray-300"><li><a href={settings.githubUrl} target="_blank" rel="noreferrer" className="focus-ring inline-flex items-center gap-2 transition-colors hover:text-accent"><Code2 className="size-4" />{copy.footer.github}</a></li><li className="group/wechat relative"><button type="button" aria-label={copy.footer.wechatHint} aria-describedby="wechat-qr" className="focus-ring inline-flex items-center gap-2 transition-colors hover:text-accent"><MessageCircle className="size-4" />{copy.footer.wechat}</button><div id="wechat-qr" className="invisible absolute bottom-full right-0 z-20 mb-4 w-44 translate-y-2 rounded-2xl border border-white/10 bg-white p-2 opacity-0 shadow-2xl shadow-black/60 transition-all duration-300 group-hover/wechat:visible group-hover/wechat:translate-y-0 group-hover/wechat:opacity-100 group-focus-within/wechat:visible group-focus-within/wechat:translate-y-0 group-focus-within/wechat:opacity-100"><img src={settings.wechatQrImage} alt={copy.footer.wechatAlt} width="648" height="633" loading="lazy" className="h-auto w-full rounded-xl" /></div></li></ul></div>
          </div>
        </motion.div>
