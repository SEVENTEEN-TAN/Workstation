    ...
    <section id="work" className="scroll-mt-20 py-24 sm:py-32">
      <div className="page-shell">
        <motion.div {...reveal} transition={{ duration: 0.7 }} className="mb-16 flex flex-col gap-7 sm:flex-row sm:items-end sm:justify-between">
          ...
          <button type="button" className="text-link focus-ring group self-start sm:self-auto">{copy.works.viewAll}<ArrowUpRight className="size-4 transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></button>
        </motion.div>
          ...
          <div className="lg:col-span-5 lg:pl-8">
            <AnimatePresence mode="wait">
              <motion.div key={activeProject.title} initial={{ opacity: 0, y: reduceMotion ? 0 : 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: reduceMotion ? 0 : -16 }} transition={{ duration: reduceMotion ? 0.01 : 0.45, ease: luxuryEase }}>
                ...
                <button type="button" className="primary-button focus-ring group mt-10">{copy.works.explore}<ArrowUpRight className="size-4 transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></button>
              </motion.div>
            </AnimatePresence>
          </div>
        </div>
