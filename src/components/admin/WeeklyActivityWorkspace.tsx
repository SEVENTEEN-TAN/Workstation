...
export function writeRecovery(id: string, values: WeeklyDraftCopy, expectedUpdatedAt: string) {
  try {
...
export function WeeklyActivityWorkspace({ initialDrafts }: { initialDrafts: WeeklyActivityDraftData[] }) {
  ...
  async function convert(draft: WeeklyActivityDraftData) {
    ...
    if (editing && dirty && editing.draft.id !== draft.id) {
      ...
      if (!writeRecovery(editing.draft.id, editing.values, editing.baseUpdatedAt)) {
        setConversionNotice("浏览器未能保存当前周报的修改。请先保存或取消编辑，再转换其他周报。");
        return;
      }
    }
    ...
    <section>
      ...
      <section className={styles.panel}>
        ...
        <form className={styles.entityForm} onSubmit={generate}>
          <label><span>开始日期</span><input name="weekStart" type="date" required defaultValue={week.start} disabled={converting} /></label>
          <label><span>结束日期</span><input name="weekEnd" type="date" required defaultValue={week.end} disabled={converting} /></label>
          <div className={styles.entityFormActions}>
            <button className={styles.primaryButton} disabled={generating || saving || converting}>{generating ? <LoaderCircle className={styles.spin} size={17} /> : <RefreshCw size={17} />}{generating ? "处理中" : "生成或打开周报"}</button>
          </div>
        </form>
      ...
      {editing ? (
        <section className={styles.panel} aria-labelledby="weekly-editor-title">
          ...
          <form className={styles.entityForm} onSubmit={save}>
            <label><span>中文标题</span><input name="titleZh" required maxLength={120} value={editing.values.titleZh} onChange={(event) => updateValue("titleZh", event.currentTarget.value)} disabled={converting} /></label>
            <label><span>英文标题</span><input name="titleEn" maxLength={120} value={editing.values.titleEn} onChange={(event) => updateValue("titleEn", event.currentTarget.value)} disabled={converting} /></label>
            <label><span>中文摘要</span><textarea name="summaryZh" required maxLength={4000} value={editing.values.summaryZh} onChange={(event) => updateValue("summaryZh", event.currentTarget.value)} disabled={converting} /></label>
            <label><span>英文摘要</span><textarea name="summaryEn" maxLength={4000} value={editing.values.summaryEn} onChange={(event) => updateValue("summaryEn", event.currentTarget.value)} disabled={converting} /></label>
            <div className={styles.entityFormActions}>
          ...
          {editing.candidate ? <article className={styles.aiDraftCard}>
            <div className={styles.aiDraftHeader}>
              ...
              <button type="button" className={styles.primaryButton} disabled={candidateStale || converting} onClick={adoptCandidate}><Check size={15} />采用此候选</button>
            </div>
        ...
        {drafts.length ? <div className={styles.activityList}>{drafts.map((draft) => {
          ...
          <article key={draft.id} className={styles.activityCard}>
            ...
            <div className={styles.rowActions}>
              {draft.status === "DRAFT" ? <button type="button" onClick={() => openEditor(draft)} disabled={converting}><FilePenLine size={15} />编辑</button> : null}
              {draft.status === "DRAFT" ? <button type="button" onClick={() => rewriteWithAi(draft)} disabled={converting || isBusy(`weekly:ai:${draft.id}`)}>{isBusy(`weekly:ai:${draft.id}`) ? <LoaderCircle className={styles.spin} size={15} /> : <Sparkles size={15} />}AI 候选</button> : null}
