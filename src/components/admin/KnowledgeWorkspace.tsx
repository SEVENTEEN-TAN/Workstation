...
import type { AssetData, KnowledgeArticleData, KnowledgeNoteData, KnowledgeNoteLinkData, KnowledgePublicationDraftData, KnowledgeSourceRevisionData, KnowledgeSyncChangeData, KnowledgeSyncReportData, KnowledgeVaultData, OkrCycleData } from "./types";
...
export function KnowledgeWorkspace({ initialVaults }: { initialVaults: KnowledgeVaultData[] }) {
  ...
  const publicationEmbeds = viewingNote ? selectedLinks.filter((link) => link.kind === "EMBED" && link.sourceRelativePath === viewingNote.relativePath).map((link) => link.targetRaw) : [];
  ...
  async function selectAttachment(draft: KnowledgePublicationDraftData, target: string, assetId: string) {
    ...
    setVaults((current) => current.map((vault) => vault.id !== selected.id ? vault : {
      ...
      sourceRevisions: vault.sourceRevisions.map((revision) => revision.draft?.id !== draft.id ? revision : {
        ...revision,
        draft: { ...revision.draft, attachments: [...revision.draft.attachments.filter((item) => item.target !== attachment.target), attachment] },
      }),
    }));
  }
